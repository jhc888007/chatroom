...
int main(int argc,char **argv)
{
    ...
    if ((socket_id = socket(AF_INET,SOCK_STREAM,0)) < 0) {
        MLOG(ERROR, "Socket Add Error: %d", socket_id);
        exit(0);
    }
    MLOG(INFO, "Socket Add: %d", socket_id);
    ...
    if ((connect(socket_id,(struct sockaddr*)&sockaddr,sizeof(sockaddr))) < 0) {
        MLOG(ERROR, "Socket Connect Error %s Errno: %d", strerror(errno),errno);
        exit(0);
    }
